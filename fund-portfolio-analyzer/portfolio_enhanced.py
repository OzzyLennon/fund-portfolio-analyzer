#!/usr/bin/env python3
"""
A-Share Fund Portfolio Analyzer - Enhanced v5
=============================================
新增功能：
  1. 同类排名：基金在同类中的百分位排名
...
def calc_metrics(history):
    ...
    drawdown = float(((nav_series - peak) / peak * 100).min())
...
def generate_report(results, opt):
    lines = []
    now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M')
    lines.append("# 📊 基金组合增强分析报告 v5")
    lines.append("**时间：" + now + "**")
    lines.append("")
    lines.append("## 一、综合持有评分")
    lines.append("")
    lines.append("| 基金 | 类型 | 评分 | 建议 | 年化(含费) | 夏普 | 同类前% |")
    lines.append("|------|------|------|------|---------|------|---------|")
    ...
    for r in scored:
        ...
        lines.append("| " + r['name'][:10] + " | " + r.get('fund_type','mixed') + " | " + icon + str(score) + " | " + tag + " | " + net + " | " + sharpe + " | " + pct + "% |")
    lines.append("")
    ...
    lines.append("## 五、风险指标")
    lines.append("")
    lines.append("| 基金 | 年化收益 | 波动率 | Sortino | 最大回撤 |")
    lines.append("|------|---------|--------|---------|---------|")
    for r in scored:
        m = r.get('metrics', {})
        ann = str(round(m.get('annualized', 0), 1)) + '%'
        ...
        lines.append("| " + r['name'][:10] + " | " + ann + " | " + vol + " | " + sortino + " | " + mdd + " |")
    lines.append("")
    return '\n'.join(lines)
